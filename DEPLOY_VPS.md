# 🚀 Обновление проекта на VPS

## Быстрое обновление

### Вариант 1: Автоматический скрипт (рекомендуется)

1. **Подключитесь к VPS по SSH:**
   ```bash
   ssh user@your-vps-ip
   ```

2. **Перейдите в директорию проекта:**
   ```bash
   cd /path/to/public
   ```

3. **Скачайте скрипт обновления (если его еще нет):**
   ```bash
   # Скрипт должен быть в репозитории, если нет - скопируйте deploy-update.sh на VPS
   ```

4. **Сделайте скрипт исполняемым и запустите:**
   ```bash
   chmod +x deploy-update.sh
   ./deploy-update.sh
   ```

### Вариант 2: Ручное обновление

1. **Подключитесь к VPS:**
   ```bash
   ssh user@your-vps-ip
   cd /path/to/public
   ```

2. **Получите последние изменения из Git:**
   ```bash
   git fetch origin
   git pull origin main
   ```

3. **Остановите текущие контейнеры:**
   ```bash
   docker compose down
   ```

4. **Пересоберите образы:**
   ```bash
   docker compose build --no-cache
   ```

5. **Запустите контейнеры:**
   ```bash
   docker compose up -d
   ```

6. **Проверьте статус:**
   ```bash
   docker compose ps
   docker compose logs --tail=50 backend
   ```

## ⚠️ Важные замечания

### Перед обновлением:

1. **Проверьте, что файл `.env` сохранен** - он содержит важные API ключи и не находится в Git
2. **Сделайте backup** базы данных Redis (если нужно):
   ```bash
   docker compose exec redis redis-cli BGSAVE
   ```

### После обновления:

1. **Проверьте работоспособность:**
   - Frontend: `http://your-vps-ip:3000`
   - Backend API: `http://your-vps-ip:8000/docs`
   - Admin панель: `http://your-vps-ip:3001`

2. **Проверьте логи на ошибки:**
   ```bash
   docker compose logs backend
   docker compose logs frontend
   ```

## 🔄 Откат изменений (если что-то пошло не так)

Если после обновления возникли проблемы:

1. **Используйте backup, созданный скриптом:**
   ```bash
   docker compose down
   cd ..
   rm -rf public
   cp -r backup_YYYYMMDD_HHMMSS public
   cd public
   docker compose up -d
   ```

2. **Или откатите изменения через Git:**
   ```bash
   git log  # найдите хеш предыдущего рабочего коммита
   git reset --hard <commit-hash>
   docker compose build --no-cache
   docker compose up -d
   ```

## 📋 Чек-лист перед обновлением

- [ ] Подключение к VPS работает
- [ ] Файл `.env` сохранен и не будет перезаписан
- [ ] Есть доступ к репозиторию GitHub (токен настроен)
- [ ] Docker и Docker Compose установлены и работают
- [ ] Сделана резервная копия важных данных (если нужно)

## 🆘 Решение проблем

### Ошибка "Permission denied" при git pull:
```bash
# Проверьте права доступа
ls -la
# Убедитесь, что у пользователя есть права на запись
```

### Ошибка при сборке Docker образов:
```bash
# Очистите кэш Docker
docker system prune -a
# Попробуйте снова
docker compose build --no-cache
```

### Контейнеры не запускаются:
```bash
# Проверьте логи
docker compose logs
# Проверьте статус
docker compose ps -a
# Проверьте использование портов
netstat -tulpn | grep -E '3000|8000|3001'
```











